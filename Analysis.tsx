...
const Analysis: React.FC<AnalysisProps> = ({ transactions }) => {
  ...
  const transactionFingerprint = useMemo(
    () => transactions.map((t) => `${t.id}:${t.type}:${t.amount}:${t.category}:${t.date}`).join('|'),
    [transactions],
  );
  ...
  const healthMetrics: HealthMetric[] = useMemo(() => {
    ...
    const savingsScore = savingsRate >= 20 ? 25 : savingsRate >= 10 ? 18 : savingsRate >= 5 ? 10 : Math.max(0, Math.round(savingsRate * 2));
    const savingsStatus: HealthMetric['status'] = savingsRate >= 20 ? 'excellent' : savingsRate >= 10 ? 'good' : savingsRate >= 5 ? 'warning' : 'danger';
    ...
    const expenseStatus: HealthMetric['status'] = expenseRatio <= 50 ? 'excellent' : expenseRatio <= 70 ? 'good' : expenseRatio <= 85 ? 'warning' : 'danger';

    const categoryCount = expenseBreakdown.length;
    const topPct = topExpenseCategory && stats.expenses > 0 ? (topExpenseCategory.value / stats.expenses) * 100 : 100;
    const diverseScore = categoryCount >= 4 && topPct < 40 ? 25 : categoryCount >= 3 && topPct < 50 ? 18 : categoryCount >= 2 ? 12 : 5;
    const diverseStatus: HealthMetric['status'] = categoryCount >= 4 && topPct < 40 ? 'excellent' : categoryCount >= 3 && topPct < 50 ? 'good' : categoryCount >= 2 ? 'warning' : 'danger';
    ...
    const consistStatus: HealthMetric['status'] = incomeEntries >= 3 ? 'excellent' : incomeEntries >= 2 ? 'good' : incomeEntries >= 1 ? 'warning' : 'danger';
    ...
    return [
      {
        ...
        maxScore: 25,
        icon: 'savings',
        description: `ออมได้ ${savingsRate}% ของรายรับ`,
      ...
      },
      {
        ...
        description: `ใช้จ่าย ${Math.round(expenseRatio)}% ของรายรับ`,
      ...
      },
      {
        ...
        maxScore: 25,
        icon: 'pie_chart',
        description: `กระจายค่าใช้จ่าย ${categoryCount} หมวดหมู่`,
      ...
      },
      {
        ...
        maxScore: 25,
        icon: 'event_repeat',
        description: `บันทึกรายรับ ${incomeEntries} รายการ`,
        status: consistStatus,
      },
    ];
  }, [expenseBreakdown, stats, topExpenseCategory, transactions]);
  ...
  useEffect(() => {
    ...
    void fetchAnalysis();
  }, [expenseBreakdown.length, hasData, healthMetrics, stats, topExpenseCategory, totalScore, transactionFingerprint]);

  const radarData = healthMetrics.map((m) => ({ subject: m.label, score: m.score, fullMark: m.maxScore }));

  const getStatusColor = (status: HealthMetric['status']) => {
    if (status === 'excellent') return { text: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-100 dark:bg-emerald-950', bar: 'bg-emerald-500' };
    if (status === 'good') return { text: 'text-blue-600 dark:text-blue-400', bg: 'bg-blue-100 dark:bg-blue-950', bar: 'bg-blue-500' };
    if (status === 'warning') return { text: 'text-yellow-600 dark:text-yellow-400', bg: 'bg-yellow-100 dark:bg-yellow-950', bar: 'bg-yellow-500' };
    return { text: 'text-red-600 dark:text-red-400', bg: 'bg-red-100 dark:bg-red-950', bar: 'bg-red-500' };
  };

  const getCategoryColor = (name: string, index: number) => CATEGORY_COLORS[name] || CHART_COLORS[index % CHART_COLORS.length];

  if (!hasData) {
    return (
      <div className="w-full min-h-screen bg-white dark:bg-black">
        <div className="max-w-[920px] mx-auto px-4 sm:px-6 lg:px-8 py-10">
          <div className="rounded-2xl border border-gray-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-8 text-center shadow-sm">
            <div className="mx-auto mb-4 size-14 rounded-2xl bg-gray-100 dark:bg-neutral-800 flex items-center justify-center">
              <span className="material-symbols-outlined text-gray-700 dark:text-neutral-200">query_stats</span>
            </div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">ยังไม่มีข้อมูลให้วิเคราะห์</h1>
            <p className="mt-2 text-sm text-gray-600 dark:text-neutral-400">เริ่มจากการบันทึกรายรับและรายจ่ายก่อน แล้วระบบจะคำนวณคะแนนสุขภาพการเงินให้ทันที</p>
      ...
      <div className="w-full max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-10 py-8 flex flex-col gap-6 lg:gap-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-neutral-400">สรุปจุดแข็ง จุดที่ควรปรับ และคำแนะนำที่ทำได้จริง</p>
          </div>
        </div>
          ...
          <div className="p-6 rounded-lg bg-white dark:bg-neutral-900 border border-gray-200 dark:border-neutral-800 shadow-sm flex flex-col items-center justify-center gap-4">
            <div className="relative size-40">
              <svg className="size-full -rotate-90" viewBox="0 0 36 36">
                <path className="text-gray-200 dark:text-neutral-700" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="currentColor" strokeWidth="2.5" />
                <path className={totalScore >= 80 ? 'text-emerald-500' : totalScore >= 60 ? 'text-blue-500' : totalScore >= 40 ? 'text-yellow-500' : 'text-red-500'} d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="currentColor" strokeDasharray={`${totalScore}, 100`} strokeWidth="2.5" strokeLinecap="round" />
              </svg>
            ...
            <div className={`px-3 py-1 rounded-full text-sm font-bold ${overallStatus.bg} ${overallStatus.color}`}>{overallStatus.label}</div>
          </div>
              ...
              {expenseBreakdown.map((item, index) => (
                <div key={item.name} className="flex items-center justify-between text-xs">
                  <div className="flex items-center gap-1.5">
                    <div className="size-2 rounded-full" style={{ backgroundColor: getCategoryColor(item.name, index) }} />
        ...
        <section className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <MiniStat label="เงินลงทุนรวม" value={`฿${stats.invest.toLocaleString()}`} icon="finance" color="blue" />
          <MiniStat label="คงเหลือสุทธิ" value={`฿${stats.net.toLocaleString()}`} icon="account_balance" color="purple" />
        </section>
          ...
          <div className="w-full rounded-lg border border-gray-200 dark:border-neutral-800 bg-gray-50 dark:bg-neutral-900 shadow-sm p-6 flex flex-col gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-gray-500 dark:text-neutral-400 text-xs">วิเคราะห์เชิงลึกจากข้อมูลจริงของคุณ</p>
              </div>
            </div>

            {isLoadingAI ? (
              <div className="text-sm text-gray-500 dark:text-neutral-400">AI กำลังวิเคราะห์ข้อมูลของคุณ...</div>
            ) : (
            ...
            <div className="flex flex-col sm:flex-row gap-3 mt-2">
              <Link to="/chat" className="flex items-center justify-center gap-2 px-5 py-2.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-sm transition-all">
                <span className="material-symbols-outlined text-[18px]">chat</span>
                ถาม AI เพิ่มเติม
              </Link>
              <Link to="/entry" className="flex items-center justify-center gap-2 px-5 py-2.5 rounded-lg border border-gray-300 dark:border-neutral-700 bg-white dark:bg-neutral-800 hover:bg-gray-50 dark:hover:bg-neutral-700 text-gray-700 dark:text-neutral-300 font-bold text-sm transition-all">
